...
export default function ImageProcessorPage() {
  ...
  return (
    <div className="min-h-screen">
      <h1 className="text-2xl font-bold text-gray-900 mb-1">Smart Image Processor</h1>
      <p className="text-sm text-gray-500 mb-6">Remove background, add a suit overlay, and set your background — all in your browser. Your image never leaves your device.</p>

      {step === 'upload' && (
        <div
          onDrop={handleDrop}
          onDragOver={handleDragOver}
          className="border-2 border-dashed border-gray-300 rounded-2xl p-16 text-center cursor-pointer hover:border-khmer-blue transition-colors bg-white/50"
        ...
        >
          ...
          <p className="text-sm text-gray-400">Supports JPG, PNG • Max 20 MB</p>
          ...
          {isProcessing && (
            <div className="mt-6 flex flex-col items-center gap-3">
              <div className="w-10 h-10 border-4 border-khmer-blue border-t-transparent rounded-full animate-spin" />
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          <div className="flex-1">
            ...
            >
              ...
              {currentSuit && (
                ...
                >
                  ...
                  />
                  <div data-rotate className="absolute -top-8 left-1/2 -translate-x-1/2 w-6 h-6 bg-white rounded-full border-2 border-khmer-blue cursor-grab active:cursor-grabbing flex items-center justify-center shadow-sm">
                    <svg className="w-3 h-3 text-khmer-blue" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                  </div>
                  <div data-resize="tl" className="absolute -top-2 -left-2 w-4 h-4 bg-white border-2 border-khmer-blue rounded-sm cursor-nw-resize shadow-sm" />
                  <div data-resize="tr" className="absolute -top-2 -right-2 w-4 h-4 bg-white border-2 border-khmer-blue rounded-sm cursor-ne-resize shadow-sm" />
                  <div data-resize="bl" className="absolute -bottom-2 -left-2 w-4 h-4 bg-white border-2 border-khmer-blue rounded-sm cursor-sw-resize shadow-sm" />
                  <div data-resize="br" className="absolute -bottom-2 -right-2 w-4 h-4 bg-white border-2 border-khmer-blue rounded-sm cursor-se-resize shadow-sm" />
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between mt-3">
              ...
              <p className="text-xs text-gray-400">Scroll to zoom • Drag to pan • Drag suit to position</p>
            </div>
          </div>
          ...
          <div className="w-full lg:w-72 space-y-5">
            <div>
              ...
              <div className="flex gap-2">
                <button onClick={() => handleGenderChange('male')} className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium border transition-colors ${gender === 'male' ? 'bg-khmer-blue text-white border-khmer-blue' : 'bg-white text-gray-700 border-gray-200 hover:border-gray-300'}`}>Male</button>
                <button onClick={() => handleGenderChange('female')} className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium border transition-colors ${gender === 'female' ? 'bg-khmer-blue text-white border-khmer-blue' : 'bg-white text-gray-700 border-gray-200 hover:border-gray-300'}`}>Female</button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {SUITS.filter(s => s.gender === gender).map(s => (
                  <button key={s.id} onClick={() => setSuitId(s.id)} className={`relative aspect-[3/4] rounded-xl border-2 overflow-hidden transition-all ${suitId === s.id ? 'border-khmer-blue ring-2 ring-khmer-blue/30 shadow-md' : 'border-gray-200 hover:border-gray-300 hover:shadow-sm'}`}>
                    <div className="w-full h-full bg-white" dangerouslySetInnerHTML={{ __html: s.svg.replace('<svg', '<svg style="width:100%;height:100%"') }} />
                    <span className="absolute bottom-0 inset-x-0 text-xs font-semibold text-center py-1.5 bg-gradient-to-t from-white via-white/95 to-transparent text-gray-800">{s.name}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {BG_OPTIONS.map(b => (
                  <button key={b.id} onClick={() => setBgId(b.id)} className={`aspect-square rounded-lg border-2 overflow-hidden transition-all ${bgId === b.id ? 'border-khmer-blue ring-2 ring-khmer-blue/20' : 'border-gray-100 hover:border-gray-200'}`} title={b.name}>
            ...
            <button onClick={generatePreview} className="w-full py-2.5 rounded-xl text-sm font-semibold border-2 border-khmer-blue text-khmer-blue hover:bg-khmer-blue hover:text-white transition-all flex items-center justify-center gap-2">
            ...
            <button onClick={() => { setStep('upload'); setCleanUrl(null); resetView() }} className="w-full text-sm text-gray-400 hover:text-gray-600 transition-colors">
              Start over
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => { setShowPreview(false); if (previewUrl) URL.revokeObjectURL(previewUrl) }}>
          <div className="bg-white rounded-2xl shadow-2xl overflow-hidden max-w-lg w-full" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
              ...
              <button onClick={() => { setShowPreview(false); if (previewUrl) URL.revokeObjectURL(previewUrl) }} className="text-gray-400 hover:text-gray-600 transition-colors">
            ...
            <div className="p-5 flex justify-center">
              {previewUrl ? (
                ...
                <div className="w-60 h-80 bg-gray-100 rounded-lg flex items-center justify-center text-sm text-gray-400">Generating preview...</div>
              )}
            </div>
            <div className="px-5 pb-5 flex gap-2">
              ...
              <button onClick={() => { setShowPreview(false); if (previewUrl) URL.revokeObjectURL(previewUrl) }} className="px-4 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors">
                Close
              </button>
            </div>
          </div>
