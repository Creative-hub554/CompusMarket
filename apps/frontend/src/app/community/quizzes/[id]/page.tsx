...
interface ResultData {
  score: number;
  answers: { id: string; questionId: string; answer: string; isCorrect: boolean }[];
}
...
export default function QuizDetailPage() {
  ...
  async function updateQuiz() {
    try {
      ...
      loadQuiz();
    } catch (err) { console.error("Failed to update quiz:", err); }
  }
          ...
          {questions.map((q, i) => (
            <div key={q.id} className={`border rounded-xl p-5 transition ${answers[q.id] ? "border-khmer-blue/20 bg-blue-50/30" : "border-gray-200"}`}>
              ...
              {q.type === "TRUE_FALSE" ? (
                ...
                <input value={answers[q.id] || ""} onChange={(e) => setAnswers((a) => ({ ...a, [q.id]: e.target.value }))} placeholder="Type your answer..." className="input-field" />
              ) : (
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="space-y-4">
          {result.answers?.map((answer: any) => {
            ...
            if (!q) return null;
            return (
              <div key={answer.id} className={`rounded-xl border p-5 ${answer.isCorrect ? "border-green-200 bg-green-50/50" : "border-red-200 bg-red-50/50"}`}>
                <div className="flex items-start gap-3">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${answer.isCorrect ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}>
                    {answer.isCorrect ? (
        ...
        <div className="mt-6 flex gap-3">
          <button onClick={() => { setMode("view"); setAnswers({}); setAttemptId(null); }} className="btn-primary">Back to Quiz</button>
          <Link href="/community/quizzes" className="btn-ghost">All Quizzes</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-start justify-between mb-6">
          ...
          {editingTitle ? (
            <div className="mt-2 space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={updateQuiz} className="btn-success">Save</button>
                <button onClick={() => setEditingTitle(false)} className="btn-ghost">Cancel</button>
              </div>
            </div>
          ) : (
            <>
        ...
        <div className="flex gap-2 ml-4 flex-wrap shrink-0">
          {!editingTitle && <button onClick={() => setEditingTitle(true)} className="btn-ghost">Edit Info</button>}
          <button onClick={startQuiz} disabled={questions.length === 0} className="btn-primary">
          ...
          <button onClick={deleteQuiz} className="btn-danger">Delete</button>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={loadAttempts} className="btn-ghost flex items-center gap-1">
              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
              {showAttempts ? "Hide" : "Attempts"}
            </button>
            <button onClick={() => setShowAddQuestion(!showAddQuestion)} className="btn-primary text-xs px-3 py-1.5">
        ...
        {showAddQuestion && (
          <div className="border border-gray-200 rounded-xl p-5 bg-gray-50/50 space-y-3 animate-slide-down">
            ...
            <input value={qText} onChange={(e) => setQText(e.target.value)} placeholder="Question text" className="input-field" />
            {qType === "MULTIPLE_CHOICE" && (
              <textarea value={qOptions} onChange={(e) => setQOptions(e.target.value)} placeholder="Options (one per line)" className="input-field" rows={3} />
            )}
            <input value={qAnswer} onChange={(e) => setQAnswer(e.target.value)} placeholder="Correct answer" className="input-field" />
            <button onClick={addQuestion} disabled={!qText.trim() || !qAnswer.trim()} className="btn-success">Add Question</button>
          </div>
        )}
