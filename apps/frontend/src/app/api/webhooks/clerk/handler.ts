 ...
 * DB-authoritative (clerkId is only the link), so this module mirrors identity
 * fields — and Clerk bans, which map onto the BANNED role. Unbanning is
 * deliberately NOT automatic: only an admin changing the role in the app can
 * restore access.
 *
...
function identityOf(data: UserEventData) {
...
async function upsertFromClerk(data: UserEventData) {
  ...
  if (identity.email) {
    ...
    if (byEmail) {
      ...
      if (banned && byEmail.role !== "BANNED") {
        return updateWithBan(byEmail, identity, {
          clerkId: data.id,
...
/** Applies a verified Clerk user lifecycle event to the local database. */
export async function handleWebhookEvent(evt: WebhookEvent): Promise<void> {
  switch (evt.type) {
    case "user.created":
    case "user.updated":
