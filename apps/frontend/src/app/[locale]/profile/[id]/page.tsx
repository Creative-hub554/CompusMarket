...
export default function ProfilePage() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      const data = await apiFetch<{ items: FeedPost[] }>(`/api/profiles/${id}/posts`);
      setPosts(data.items ?? []);
    } catch {
      setPosts([]);
    }
  ...
  const refresh = useCallback(async () => {
    const [p] = await Promise.all([
      apiFetch<Profile>(`/api/profiles/${id}`).catch(() => null),
      loadPosts(),
    ]);
    if (p) setProfile(p);
  }, [id, loadPosts]);
    ...
    Promise.all([
      ...
      .then(async ([p]) => {
        ...
        if (p?.id && p.id === session?.user?.id) {
          try {
            const data = await apiFetch<FollowRequest[]>("/api/follow-requests");
  ...
  async function respondToRequest(requestId: string, accept: boolean) {
    try {
      await apiFetch(`/api/follow-requests/${requestId}/${accept ? "accept" : "decline"}`, {
        method: "POST",
      });
    } catch {
      return; // non-2xx responses used to stop silently
    }
    ...
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      {locked ? (
        ...
        <div className="space-y-5">
          {posts.map((post) => (
            <ErrorBoundary key={post.id} label={`profile post ${post.id}`}>
              <PostCard
                post={post}
