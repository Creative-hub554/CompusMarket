...
export default async function Home() {
  ...
  return (
    <div>
      {/* Hero: mesh gradient + floating glass chips */}
      <section className="gradient-mesh text-white">
        <div className="mx-auto max-w-7xl px-4 py-20 md:py-28 grid md:grid-cols-2 gap-12 items-center relative">
          <div className="animate-fade-in-up relative z-10">
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Link
                href="/shop"
                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 text-white px-8 py-3.5 font-bold shadow-[0_8px_30px_-6px_rgba(99,102,241,0.6)] hover:shadow-[0_10px_36px_-6px_rgba(99,102,241,0.8)] hover:-translate-y-0.5 transition-all"
              >
                {t("browseShop")}
                <ArrowRight size={18} />
              </Link>
              <Link
                href="/seller/apply"
                className="inline-block rounded-xl border border-white/25 bg-white/5 backdrop-blur text-white/90 px-8 py-3.5 font-semibold hover:bg-white/10 hover:-translate-y-0.5 transition-all"
              >
                {t("becomeSeller")}
              </Link>
            </div>
          ...
          <div className="hidden md:block relative h-96 animate-fade-in-up" style={{ animationDelay: "0.15s" }}>
            <div className="absolute inset-x-8 top-1/2 -translate-y-1/2 glass-card !bg-white/10 !border-white/20 p-10 flex flex-col items-center gap-5">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                ...
                width={120}
                height={120}
                className="animate-temple-float drop-shadow-[0_0_30px_rgba(139,92,246,0.6)]"
              />
              <p className="text-white/80 text-sm text-center max-w-[16rem]">
                {t("tilesCaption")}
              </p>
            </div>
            <div className="absolute -top-1 right-2 glass-card !bg-white/10 !border-white/20 !rounded-xl px-4 py-2.5 flex items-center gap-2 animate-temple-float" style={{ animationDelay: "0.8s" }}>
              <MessagesSquare size={16} className="text-indigo-300" />
              <span className="text-xs font-semibold text-white/90">{nav("social")}</span>
            </div>
            <div className="absolute bottom-2 left-0 glass-card !bg-white/10 !border-white/20 !rounded-xl px-4 py-2.5 flex items-center gap-2 animate-temple-float" style={{ animationDelay: "1.6s" }}>
      ...
      <section className="border-b border-[var(--border-subtle)] bg-[var(--surface)]">
        <div className="mx-auto max-w-7xl px-4 py-6 grid grid-cols-3 gap-4">
          {pillars.map(({ href, label, Icon }) => (
            ...
            >
              <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500/15 to-violet-500/15 text-indigo-500 dark:text-indigo-300">
                <Icon size={18} />
              </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12 stagger-children">
            {features.map(({ title, desc, Icon }) => (
              <div key={title} className="bento-card p-9 text-left">
                <span className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white shadow-[0_8px_20px_-6px_rgba(99,102,241,0.55)]">
                  <Icon size={22} />
                </span>
      ...
      {/* CTA */}
      <section className="gradient-mesh text-white">
        <div className="mx-auto max-w-4xl px-4 py-20 text-center relative z-10">
          <h2 className="text-3xl md:text-4xl font-extrabold mb-4 tracking-tight">
            {t("becomeSellerDesc")}
          </h2>
          <Link
            href="/seller/apply"
            className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 to-violet-600 text-white px-10 py-3.5 font-bold shadow-[0_8px_30px_-6px_rgba(99,102,241,0.6)] hover:-translate-y-0.5 transition-all"
          >
            {t("applyNow")}
            <ArrowRight size={18} />
          </Link>
