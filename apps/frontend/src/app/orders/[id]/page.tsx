...
export default function OrderDetailPage() {
  ...
  if (loading)
    return <div className="mx-auto max-w-3xl px-4 py-8">{t("loading")}</div>;
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <Link
                    ...
                    className="font-medium hover:text-khmer-blue"
                  >
                    {item.product.name}
                  </Link>
                  <span
                ...
                {item.trackingNumber && (
                  <p className="text-xs text-gray-400 mt-1">
                    {t("tracking", { number: item.trackingNumber })}
                  </p>
                )}
            ...
            {item.status === "DELIVERED" && !item.feedback && (
              <FeedbackForm
                onSubmit={(rating, comment, images) =>
                  submitFeedback(item.id, rating, comment, images)
                }
              />
            )}
