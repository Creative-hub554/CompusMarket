...
export class QuizzesController {
  ...
  @Post()
  create(@Req() req: { user: { userId: string } }, @Body() body: { title: string; description?: string; public?: boolean; timeLimit?: number }) {
  ...
  @Patch(":id")
  update(@Req() req: { user: { userId: string } }, @Param("id") id: string, @Body() body: { title?: string; description?: string; public?: boolean; timeLimit?: number }) {
  ...
  addQuestion(@Req() req: { user: { userId: string } }, @Param("quizId") quizId: string, @Body() body: { type: QuestionType; question: string; options?: string[]; correctAnswer: string; points?: number; order?: number }) {
    return this.quizzesService.addQuestion(quizId, req.user.userId, body);
  }

  @Patch("questions/:id")
  updateQuestion(@Req() req: { user: { userId: string } }, @Param("id") id: string, @Body() body: { type?: QuestionType; question?: string; options?: string[]; correctAnswer?: string; points?: number; order?: number }) {
  ...
  submitAnswer(@Req() req: { user: { userId: string } }, @Param("attemptId") attemptId: string, @Body() body: { questionId: string; answer: string }) {
